...
public class DBOperation {
	...
	public static int exchangeWeiJin(WeiJinEntity weijin)
	{
		...
		if(i==1)
		{
			 t = update("WeiJin","ExchangeTime = '"+date+"'","WeiJinId = "+weijin.getWeiJinIDString());
		}
		int j = 0,k=0;
		if(t==1)
		{
	...
	{
		String userId;
		ArrayList<ArrayList<String>> userIdArray = new ArrayList<ArrayList<String>>();
		userIdArray = select ( "WUser", "UserId", "Where Email = '"+email+"'");
		if(userIdArray.size()==0)
			return null;
		else
		{
			userId = userIdArray.get(0).get(0);
			return userId;
		}
	}
